...
//Function Prototype
Board readFromFile(string filename);
...
int main(int argv, char **argc)
{
    ...
    //Initialize Board
    board = readFromFile("board_data.txt"); //Function to read board configuration from a file and initialize the board accordingly
    board.initializeDeckManually(); //Function to initialize the Chance and Community Chest decks with the appropriate 
    writeToFile("board_data.txt", board); //Function to write the current board configuration to a file (to be implemented)
...
void writeToFile(string filename, Board& board) {
    ...
    outFile.open(filename, ios::out | ios::binary); //Open the file for writing in binary mode
        ...
        case 1:
            ...
            if (dice.isDouble()) { //Check if the player rolled doubles
                ...
                if (plyrs[plyrInd].getJailTurns() >= 3) { //If this is the player's third turn in jail, they must pay the fine and get out of jail
                    ...
                    plyrs[plyrInd] -= 50; //Function to deduct a certain amount from the player's balance for paying the fine
