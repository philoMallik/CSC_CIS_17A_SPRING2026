...
void initialization(string* fileNames, City* cities, Board* board,
                    int* chanceCards, int* communityChestCards) {
...
void movePlayer(int* doublecount, bool* doubleRoll, int playerIndex, Player* players, Board* board, City* cities, int* chanceCards, int* communityChestCards, int* housesAvailable, int* hotelsAvailable, int noOfPlyrs){
    ...
    if (!players[playerIndex].inJail){
        ...
        if (players[playerIndex].position >= BOARD_SIZE){
            ...
            cout << board[players[playerIndex].position].name << " at position " << players[playerIndex].position << "." << endl;
        ...
        else cout << board[players[playerIndex].position].name << " at position " << players[playerIndex].position << "." << endl;
...
*/
    ...
    switch (board[players[playerIndex].position].type){
        ...
        case 3:
            ...
            if(cities[propertyID].ownerIndex != -1){ //If the property is owned by another player, pay rent
                ...
                cout << "You landed on " << cities[propertyID].name << ", which is unowned." << endl;
                cout << "Would you like to buy it for $" << cities[propertyID].cost << "? (y/n): " << endl;
                cin >> choice;
...
void payRentOrTax(int playerIndex, int rollTotal, Player* player, Board* board, City* cities){
    ...
    rentAmount = cities[board[player[playerIndex].position].propertyID].houseRent[cities[board[player[playerIndex].position].propertyID].upgradeLevel]; //Base rent amount based on the property's upgrade level
    ...
    switch(propertyType){
        case 1: //City
            ...
            rentAmount = cities[board[player[playerIndex].position].propertyID].houseRent[cities[board[player[playerIndex].position].propertyID].upgradeLevel];
            break;
        case 2: //Railroad
            cout<< "You landed on " << cities[board[player[playerIndex].position].propertyID].name << " owned by " << player[ownerIndex].name << ". You must pay $" << rentAmount << " in rent." << endl;
...
void calculateGroupOwned(int propertyID, int playerIndex, Player* players, City* cities){
    ...
    for (int i=0; i<MAX_CITIES; i++){
        if (cities[i].groupID == groupID){
            if (cities[i].ownerIndex == -1 || cities[i].ownerIndex != playerIndex){
        ...
        if (cities[propertyID].upgradeLevel >= 5){
            ...
            if (*hotelsAvailable <= 0){
                ...
                if (cities[propertyID].upgradeLevel > cities[propertyID].groupMinimumUpgradeLevel){
                    ...
                    moneyTransaction(playerIndex, players, -1*cities[propertyID].upgradeCost); //Handle money transaction with the bank for property upgrade (to be implemented)
                    ...
                    cout<< "You upgraded " << cities[propertyID].name << " to a hotel for $" << cities[propertyID].upgradeCost << "." << endl;
                }
            }
        }
        else{
            if (*housesAvailable <= 0){
                ...
                if (cities[propertyID].upgradeLevel > cities[propertyID].groupMinimumUpgradeLevel){
                    ...
                    cout<< "You upgraded " << cities[propertyID].name << " to a house for $" << cities[propertyID].upgradeCost << "." << endl;
                }
            }
        }
    }
...
void displayPropertyOneLiner(int propertyID, City* cities){
    cout << cities[propertyID].name << " - " << cities[propertyID].cost << "$ - Rent: " << cities[propertyID].houseRent[0] << "$ - Upgrade Cost: " << cities[propertyID].upgradeCost << "$ - Group Id: " << cities[propertyID].groupID;
}
    ...
    for(int i=0; i<players[playerIndex].numCities; i++){
        ...
        if (cities[propertyID].upgradeLevel > 0){
            if (cities[propertyID].upgradeLevel == 5){
                ...
                (*housesAvailable) += 4; //Return the 4 houses used for the upgrade back to the pool of available houses
                totalAssetsPrice += (cities[propertyID].upgradeCost * 4) + cities[propertyID].upgradeCost; //Add the value of the upgrades to the total assets price
            }
            else{
                ...
                totalAssetsPrice += cities[propertyID].upgradeCost * cities[propertyID].upgradeLevel; //Add the value of the upgrades to the total assets price
            }
        }
