...
*/
/*
* created the main structure of the program with the main game loop and function prototypes.
* The main game loop iterates through each player's turn, allowing them to take actions and move around the board until the game is over.
* The initialization function is a placeholder for setting up the game components (cities, board, chance cards, community chest cards) from external files.
* The getOrderOfPlay function is the first turn of the game where players roll dice to determine the order of play, and the rolls and player order are sorted in descending order based on the rolls.
* The handleMenuChoice function is a placeholder for handling the various actions a player can take during their turn based on their menu choice. (will be done in version 1)
* The movePlayer function simulates rolling the dice and moving the player around the board, including handling double rolls and passing Go. (will be completed in version 1)
* The locationAction function is a placeholder for handling the action when a player lands on a space on the board (e.g., buying property, paying rent, drawing cards, etc.) based on the type of space. (will be completed in version 1)

Most work was done on the game loop especially the order of rolling the dice and moving the player around the board.
The double roll logic was tricky to implement, especially that at each action done player can go to jail or bankrupt which would end their turn even if they rolled a double.
I had to make sure to check for those conditions after each action and end the turn if they occur, even if the player rolled a double.
I also had to make sure to reset the double roll count when the player does not roll a double or when their turn ends for any reason.

I stil need to figure out a way to make a graphical representation of the board and the players' positions on it.
which possibly will be done in later version and will be a representation in the console using ASCII characters to create a visual representation.
probably wont be forced to show every turn but whenever a player choose to view the board or at the end of each round.

next steps:
-Implement the logic for handling each menu choice in the handleMenuChoice function (buying properties, upgrading properties, mortgaging properties, etc.)
-Implement the logic for handlig the actions when a player lands on a space in the locationAction function (paying rent, drawing cards, etc.)
...
-Implement the logic for upgrading properties with houses and hotels, including checking for the required conditions (owning all properties in a color group, having enough money, etc.)
-Implement the logic for mortgaging and unmortgaging properties, including calculating the mortgage value and interest, and handling the restrictions on mortgaged properties (cannot collect rent, cannot be upgraded, etc.)
-Implement the logic for declaring bankruptcy, including transferring assets to creditors and removing the player from the game.
-Implement a way to determine when the game is over (e.g., when only one player is left who is not bankrupt) and declare the winner.
-Implement an initialization function that reads the data for the board, cities, chance cards, and community chest cards from external files to set up the game components.
-Implement a graphical representation of the board and the players' positions on it, possibly using ASCII characters in the console to create a visual representation of the board state.
-Implement additional features such as saving and loading game states, allowing players to customize their tokens and names, and adding more detailed statistics and information about the game state (e.g., a dashboard showing each player's properties, balance, net worth, etc.)
-Implement error handling and input validation throughout the program to ensure that invalid inputs do not cause crashes or unexpected behavior, and to provide clear feedback to the user when they enter invalid data.
*/
